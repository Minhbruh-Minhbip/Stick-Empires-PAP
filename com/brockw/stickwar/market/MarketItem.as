package com.brockw.stickwar.market
{
   ...
   public class MarketItem extends MovieClip
   {
      ...
      public var peepeepoopoo:Boolean;
      ...
      private var oldCard:OldArmoryUnitCard;
      ...
      public function setOldCard(param1:OldArmoryUnitCard) : void
      {
         this.oldCard = param1;
      }
      ...
      private function useButton(param1:Event) : void
      {
         if(this.card == null)
         {
            if(!this.oldCard.main.armourScreen.isUnlocking())
            {
               this.oldCard.equip(ItemMap.unitNameToType(this.unit),this.type,name);
            }
         }
         else if(!this.card.main.armourScreen.isUnlocking())
         {
            this.card.equip(ItemMap.unitNameToType(this.unit),this.type,name);
         }
      }
      
      private function oldUseButton(param1:Event) : void
      {
         if(!this.oldCard.main.armourScreen.isUnlocking())
         {
            this.oldCard.equip(ItemMap.unitNameToType(this.unit),this.type,name);
      ...
      public function realAutism(param1:Number, param2:Number) : void
      {
         this.mc.scaleX *= param1;
         this.mc.scaleY *= param2;
         var _loc14_:Rectangle = this.mc.getBounds(this.marketMc);
         this.mc.x -= _loc14_.left;
         this.mc.y -= _loc14_.top;
         this.mc.x += (W_WIDTH * param1 - _loc14_.width) / 2;
         this.mc.y += (W_HEIGHT * param2 - _loc14_.height) / 2;
      }
